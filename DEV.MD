...
[~] **全面性的錯誤處理強化 (尤其針對 Agent)**
    ...
    [x] 1. (Agent) 設計並實作針對網路請求、API 呼叫失敗的錯誤重試機制。 (已在 `fetchWebContent` 中實現)；API 呼叫的重試機制待評估。
    ...
    [~] 3. (Agent) 設定當發生嚴重錯誤或 Agent 執行失敗時的通知機制 (已建立 `sendNotification` 存根並整合到 `agent-script.ts` 的關鍵錯誤點)。
