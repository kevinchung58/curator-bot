...
實作: 後端使用 Octokit 套件來建立或更新（如果檔案已存在）GitHub 上的 .md 檔案。檔名格式為 YYYY-MM-DD-標題-簡短ID.md。此功能需要伺服器端設定 GITHUB_PAT 環境變數。前端會顯示成功或失敗的提示訊息（包含檢視文件連結）。
...
[~] **開發內容自動監控代理 (Agent)**
    ...
    [~] **2. Agent 核心邏輯開發 (Agent Core Logic Development)**
        [~] 2.1. 編寫 Agent 主要腳本 (例如 `agent-script.ts`)。(已建立 `src/agent/agent-script.ts` 並增強基礎結構)
        ...
        [~] 2.2. **網頁內容爬取與解析 (Web Content Scraping & Parsing)**
            [~] 2.2.1. 設計與實作爬蟲邏輯。 (Agent 腳本中已有 `fetchWebContent` 基礎實現，用於獲取單一 URL 內容。進階爬取多個連結待開發)
        ...
        [~] 2.5. **觸發 AI 處理流程 (Trigger AI Processing)**
            [~] 2.5.1. 當發現新內容時，自動呼叫現有的 `generateContentSummary` AI 流程。(Agent 腳本中 `triggerAIProcessing` 已包含呼叫 Next.js API 端點的邏輯)
            [ ] 2.5.2. **(重要) 在 Next.js 應用中建立 `/api/agent/process-content` API 端點。** 此端點接收來自 Agent 的請求，並調用 `processDiscoveredContent` Server Action (或直接 `generateContentSummary` Genkit flow)。
            [ ] 2.5.3. 妥善處理 AI 流程的成功與失敗回調 (在 Agent 端及 API 端)。
        [~] 2.6. **儲存處理結果 (Store Processed Results)**
            [~] 2.6.1. 將 Agent 發現並成功處理完的內容儲存到永久性資料庫 (Supabase `curated_content` 表)。(Agent 腳本中 `storeResultsInSupabase` 已包含 Supabase 插入邏輯)

    [ ] **3. Agent 狀態回報與監控 (Agent Status Reporting & Monitoring)**
        [ ] 3.1. 設計 Agent 運行狀態的回報機制 (例如，寫入 Supabase 特定表格，或透過 webhook)。
        [x] 3.2. (前端) 改造現有模擬的 Agent 狀態顯示，使其能讀取並顯示真實的 Agent 運行狀態。(目前為模擬，待真實狀態接入)
...
[~] **全面性的錯誤處理強化 (尤其針對 Agent)**
    ...
    [~] 2. (Agent) 實作更詳細、結構化的錯誤日誌記錄到 Supabase 或 GitHub Actions Logs。(GitHub Actions workflow 已設定，Agent 腳本包含 console.log，可進一步增強)
