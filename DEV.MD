...
[~] **開發內容自動監控代理 (Agent)**
    ...
    [x] **1. 代理架構選擇與設定 (Agent Architecture & Setup)**
        ...
        [x] 1.4. (若選擇 Supabase + GH Actions) 設定 GitHub Actions Workflow (`cron.yml`)。(已建立 `.github/workflows/cron.yml`)
    ...
    [~] **2. Agent 核心邏輯開發 (Agent Core Logic Development)**
        [~] 2.1. 編寫 Agent 主要腳本 (例如 `agent-script.ts`)。(已建立 `src/agent/agent-script.ts` 基礎結構)
            [ ] 2.1.1. (Agent 腳本) 實作連接 Supabase (或所選資料庫) 的邏輯。(已加入 `@supabase/supabase-js` 依賴，腳本中有 placeholder)
            [ ] 2.1.2. (Agent 腳本) 實作核心內容處理循環：讀取策略 -> 爬取 -> 檢查重複 -> AI處理 -> 儲存。(腳本中有 placeholder 邏輯)
        [ ] 2.2. **網頁內容爬取與解析 (Web Content Scraping & Parsing)**
            [ ] 2.2.1. 設計與實作爬蟲邏輯。 (Agent 腳本中有 `fetchWebContent` placeholder)
            [ ] 2.2.2. 實作 `robots.txt` 遵循機制。
            [ ] 2.2.3. 實作網頁主要內容提取邏輯。
            [ ] 2.2.4. 提升處理不同網頁結構的彈性與錯誤處理。
        [ ] 2.3. **整合使用者定義的搜尋策略 (Integrate User-defined Search Strategies)**
            [ ] 2.3.1. 實作從資料庫 (Supabase) 讀取使用者設定的搜尋策略。(Agent 腳本中有 `fetchStrategiesFromSupabase` placeholder)
            [ ] 2.3.2. 根據讀取的策略執行內容搜尋或網站爬取。
        [ ] 2.4. **處理重複內容 (Handle Duplicate Content)**
            [ ] 2.4.1. 實作在資料庫 (Supabase) 中檢查 URL 是否已存在的邏輯，防止重複處理。(Agent 腳本中有 `checkForDuplicates` placeholder)
        [ ] 2.5. **觸發 AI 處理流程 (Trigger AI Processing)**
            [ ] 2.5.1. 當發現新內容時，自動呼叫現有的 `generateContentSummary` AI 流程。(Agent 腳本中有 `triggerAIProcessing` placeholder，建議透過 Next.js API 端點調用)
            [ ] 2.5.2. 妥善處理 AI 流程的成功與失敗回調。
        [ ] 2.6. **儲存處理結果 (Store Processed Results)**
            [ ] 2.6.1. 將 Agent 發現並成功處理完的內容儲存到永久性資料庫 (Supabase `curated_content` 表)。(Agent 腳本中有 `storeResultsInSupabase` placeholder)
...
[~] **全面性的錯誤處理強化 (尤其針對 Agent)**
    ...
    [~] 2. (Agent) 實作更詳細、結構化的錯誤日誌記錄到 Supabase 或 GitHub Actions Logs。(GitHub Actions workflow 已設定，Agent 腳本包含 console.log)
...
[ ] **建立日誌與監控警報**
    ...
    [~] 3. (若使用 Supabase/GH Actions 方案) 設定 Uptime Kuma (或類似服務) 的心跳監控和失敗警報，或利用 GitHub Actions 的失敗通知。(GitHub Actions workflow 本身有失敗通知)
