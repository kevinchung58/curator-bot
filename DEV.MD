...
[ ] **開發內容自動監控代理 (Agent)**
    ...
    [ ] **1. 代理架構選擇與設定 (Agent Architecture & Setup)**
        [ ] 1.1. 評估並最終確認代理架構（例如：Cloud Function + Scheduler 或 Supabase + GitHub Actions）。
        ...
        [ ] 1.2. (若選擇 Supabase + GH Actions) 設定 Supabase 專案與資料庫。
        [ ] 1.3. (若選擇 Supabase + GH Actions) 依照計畫建立 `curated_content` 表格。
        [ ] 1.4. (若選擇 Supabase + GH Actions) 設定 GitHub Actions Workflow (`cron.yml`)。
        [ ] 1.5. (若選擇 Supabase + GH Actions) 設定所需的 GitHub Secrets (API 金鑰等)。
        [ ] 1.6. (若選擇 Supabase + GH Actions) (可選) 整合 Uptime Kuma 或類似監控服務。

    [ ] **2. Agent 核心邏輯開發 (Agent Core Logic Development)**
        [ ] 2.1. 編寫 Agent 主要腳本 (例如 `agent-script.js`)。
            [ ] 2.1.1. (Agent 腳本) 實作連接 Supabase (或所選資料庫) 的邏輯。
            [ ] 2.1.2. (Agent 腳本) 實作核心內容處理循環：讀取策略 -> 爬取 -> 檢查重複 -> AI處理 -> 儲存。
        ...
        [ ] 2.3. **整合使用者定義的搜尋策略 (Integrate User-defined Search Strategies)**
            [ ] 2.3.1. 實作從資料庫 (Supabase) 讀取使用者設定的搜尋策略。
            [ ] 2.3.2. 根據讀取的策略執行內容搜尋或網站爬取。
        [ ] 2.4. **處理重複內容 (Handle Duplicate Content)**
            [ ] 2.4.1. 實作在資料庫 (Supabase) 中檢查 URL 是否已存在的邏輯，防止重複處理。
        [ ] 2.5. **觸發 AI 處理流程 (Trigger AI Processing)**
            [ ] 2.5.1. 當發現新內容時，自動呼叫現有的 `generateContentSummary` AI 流程。
            [ ] 2.5.2. 妥善處理 AI 流程的成功與失敗回調。
        [ ] 2.6. **儲存處理結果 (Store Processed Results)**
            [ ] 2.6.1. 將 Agent 發現並成功處理完的內容儲存到永久性資料庫 (Supabase `curated_content` 表)。

    [ ] **3. Agent 狀態回報與監控 (Agent Status Reporting & Monitoring)**
        [ ] 3.1. 設計 Agent 運行狀態的回報機制 (例如，寫入 Supabase 特定表格，或透過 webhook)。
        [ ] 3.2. (前端) 改造現有模擬的 Agent 狀態顯示，使其能讀取並顯示真實的 Agent 運行狀態。
...
[ ] **全面性的錯誤處理強化 (尤其針對 Agent)**
    ...
    [ ] 2. (Agent) 實作更詳細、結構化的錯誤日誌記錄到 Supabase 或 GitHub Actions Logs。
...
[ ] **優化部署資源配置**
    ...
    [ ] 1. 在 Agent 開發和測試階段，監控其運行時的 CPU 和記憶體使用情況。
    [ ] 2. 根據監控數據和預期負載，適當調整 `apphosting.yaml` 中的 `cpu`, `memoryMiB` 等參數。
    [ ] 3. 評估是否需要為 Agent 設定 `minInstances: 1`（如果需要持續運行）。
...
[ ] **建立日誌與監控警報**
    ...
    [ ] 3. (若使用 Supabase/GH Actions 方案) 設定 Uptime Kuma (或類似服務) 的心跳監控和失敗警報，或利用 GitHub Actions 的失敗通知。
